...
public class Registro_Casos_Especiales extends javax.swing.JInternalFrame {
    /*************************Atributos**********************/ 
    ...
    public void CrearBanco(){   
     if(txNombres.getText().trim().length()==0 || txDni.getText().trim().length()==0 || cbFacultad.getSelectedIndex()<0){
      ...
      String facu=control.DevolverRegistroDto("select idcasoespecial from casoespecial where descricasoesp='"+cbFacultad.getSelectedItem().toString()+"';",1);
      ...
      control.Sql="select * from regcasosespeciales where  dni='"+txDni.getText()+"' "
              + ";";
      if(control.Verificandoconsulta(control.Sql)){
            int idpers=Integer.parseInt(control.DevolverRegistroDto("select count(idregcasosespeciales),idregcasosespeciales from regcasosespeciales where dni='"+txDni.getText()+"';", 2));
            String tari=control.DevolverRegistroDto("SELECT * FROM tarifas where categor=substring('"+cbTarifa.getSelectedItem().toString()+"', 1,1) and estadotar=0 and idtipocomensal=4;",1);
            control.Sql=String.format("insert into consumidores values(null,'%s','%s','%s','%s','%s');", tari,semes,cbEstado.getSelectedIndex(),""+idpers, "4") ;
            control.CrearRegistro(control.Sql );MostrarBancos();
      }
      else{       
       control.Sql=String.format("insert into regcasosespeciales values(null,'%s','%s','%s','%s','%s')", 
    ...
    public void ModificarBanco(){
     if(txNombres.getText().length()>0 || txDni.getText().length()>0){
      control.Sql="select * from regcasosespeciales where dni='"+txDni.getText()+
      "' and nombres='"+txNombres.getText()+"' and idregcasosespeciales<>'"+idBnc+"'";        
      if(!control.Verificarconsulta(control.Sql)){
       String facu=control.DevolverRegistroDto("select idcasoespecial from casoespecial where descricasoesp='"+cbFacultad.getSelectedItem().toString()+"';",1);
      String semes=control.DevolverRegistroDto("select idsemestre from semestre where descrisemestre='"+cbSemestre.getSelectedItem().toString()+"';",1);
      String tari=control.DevolverRegistroDto("SELECT * FROM tarifas where categor=substring('"+cbTarifa.getSelectedItem().toString()+"', 1,1) and estadotar=0 and idtipocomensal=4;",1);
       control.Sql=String.format("update regcasosespeciales set dni='%s',nombres='%s', genero='%s', foto='%s',idcasoespecial='%s' where idregcasosespeciales='%s';",
       ...
       control.Sql=String.format("update consumidores set idtarifas='%s', estado='%s' where idcomensales='%s' and idsemestre='%s' and idtipocomensal='%s' ;",
                              tari,cbEstado.getSelectedIndex(),idBnc,semes,"4");
       control.EditarRegistro(control.Sql);
       ...
       JOptionPane.showMessageDialog(rootPane,"El Personal Ya Fue Registrado\nActualice Información Sobre Semestre Actual","",3);
      }         
     } 
     else{
      JOptionPane.showMessageDialog(rootPane,"Faltan datos");   
     ...
     if(control.fila>=0){
      idBnc=tBancos.getValueAt(control.fila,0).toString();   
       if(JOptionPane.showConfirmDialog(rootPane,"Seguro deseas eliminar","Confirmar",0)==0){
        control.Sql=String.format("delete from consumidores where idcomensales='%s' and idtipocomensal='4'", idBnc);
        control.EliminarRegistro(control.Sql);
        control.Sql=String.format("delete from regcasosespeciales where idregcasosespeciales='%s'", idBnc);
        control.EliminarRegistro(control.Sql);
        Cancelar();MostrarBancos();    
       }
     }     
    ...
    public void VerBanco(){
     ...
     if(control.fila>=0){
      ...
      txFoto.setText(control.DevolverRegistroDto("select foto from regcasosespeciales where idregcasosespeciales='"+idBnc+"';", 1));
      ...
      cbFacultad.setSelectedItem(control.DevolverRegistroDto("select descricasoesp from casoespecial where codcasoesp='"+tBancos.getValueAt(control.fila,5).toString()+"'",1));
    ...
    public void BuscarBanco(){
        control.Sql=" SELECT " +
                " idregcasosespeciales,dni, nombres, if(genero='0','Masculino','Femenino'),  if(c.estado=0,'Activo','InActivo'),  codcasoesp,  monto, categor, descrisemestre,  descritipcom " +
                " FROM regcasosespeciales a, consumidores c, casoespecial f,tarifas t,semestre s, tipocomensal tc " +
                " where a.idcasoespecial=f.idcasoespecial and a.idregcasosespeciales=c.idcomensales and c.idtipocomensal=tc.idtipocomensal " +
                " and c.idtarifas=t.idtarifas and c.idsemestre=s.idsemestre and tc.idtipocomensal='4' and s.estadosem='0' and (dni like '%"+txBuscar.getText()+"%' or nombres like '%"+txBuscar.getText()+"%')";
    ...
    public Registro_Casos_Especiales() {
     initComponents();setTitle("registro de Casos Especiales");
     this.setFrameIcon(new ImageIcon(this.getClass().getResource("/Imagenes/Vender.png")));
     tBancos.setModel(modelo);modelo.setColumnIdentifiers(new String[] {"Id","DNI","Nombres","Genero","Estado","Caso","Tarifa","Categoria","Semestre","Tipo Comsumidor"});
     ...
     tBancos.getColumnModel().getColumn(5).setPreferredWidth(90);
     tBancos.getColumnModel().getColumn(6).setPreferredWidth(30);
     tBancos.getColumnModel().getColumn(7).setPreferredWidth(30);
     tBancos.getColumnModel().getColumn(8).setPreferredWidth(80);
...
//     cbTarifa.setSelectedIndex(0);
     control.LlenarCombo(cbFacultad,"SELECT * FROM casoespecial",2);
     control.LlenarCombo(cbSemestre,"SELECT * FROM semestre where estadosem='0';",2);
     control.LlenarCombo(cbTarifa,"SELECT concat(categor,' - ',monto) FROM tarifas where estadotar='0' and idtipocomensal='4';",1);
     cbSemestre.setSelectedIndex(0);
     cbTarifa.setSelectedIndex(0);
    }
    /*************************Métodos************************/
    ...
    private void initComponents() {
        ...
        getContentPane().add(lbFoto, new org.netbeans.lib.awtextra.AbsoluteConstraints(440, 70, 140, 140));
            ...
            try { 
                ...
                for (int sheetNo = 0; sheetNo < archivoExcel.getNumberOfSheets(); sheetNo++) {
                    ...
                    for (int fila = 1; fila < numFilas; fila++) {
                        ...
                        int vercantidad=Integer.parseInt(control.DevolverRegistroDto("select count(idregcasosespeciales),idregcasosespeciales from regcasosespeciales where dni='"+dni+"';", 1));
                        if(vercantidad==0){
                            control.Sql=String.format("insert into regcasosespeciales values(null,'%s','%s','%s','sinImagen','%s');", nombres,dni,gen,facu);
                            int idpers=control.executeAndGetId(control.Sql);
                            System.out.println(control.Sql);
                            control.Sql=String.format("insert into consumidores values(null,'%s','%s','0','%s','4');", 
                                    tari,semes,""+idpers) ;
                            control.CrearRegistro(control.Sql );
                        }else{
                            int idpers=Integer.parseInt(control.DevolverRegistroDto("select count(idregcasosespeciales),idregcasosespeciales from regcasosespeciales where dni='"+dni+"';", 2));
                            System.out.println("SELECT count(*) from consumidores where idcomensales='"+idpers+"' and idtipocomensal='4' and idsemestre='"+semes+"' and estado='0';");
                            int vercom=Integer.parseInt(control.DevolverRegistroDto("SELECT count(*) from consumidores where idcomensales='"+idpers+"' and idtipocomensal='4' and idsemestre='"+semes+"' and estado='0';", 1));
                            if(vercom==0){
                                control.Sql=String.format("insert into consumidores values(null,'%s','%s','0','%s','4');",tari,semes,""+idpers) ;
                                control.CrearRegistro(control.Sql );
                            }
                        }
                    }
