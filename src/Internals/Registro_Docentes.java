...
public class Registro_Docentes extends javax.swing.JInternalFrame {
    ...
    public void CrearBanco(){   
     if(txNombres.getText().trim().length()==0 || txDni.getText().trim().length()==0 ){
      ...
      control.Sql="select * from docentes where dni='"+txDni.getText()+"' ;";
      if(control.Verificandoconsulta(control.Sql)){
       //JOptionPane.showMessageDialog(rootPane,"El Docente ya esta Registrado en el sistema\nEs Nesecario la Actualizacion de la Informacion del Docente Para el Semestre ","",3);
          int idpers=Integer.parseInt(control.DevolverRegistroDto("select count(iddocentes),iddocentes from docentes where dni='"+txDni.getText()+"';", 2));
          String tari=control.DevolverRegistroDto("SELECT * FROM tarifas where categor=substring('"+cbTarifa.getSelectedItem().toString()+"', 1,1) and estadotar=0 and idtipocomensal=2;",1);
          control.Sql=String.format("insert into consumidores values(null,'%s','%s','%s','%s','%s');", tari,semes,cbEstado.getSelectedIndex(),""+idpers, "2") ;
          control.CrearRegistro(control.Sql );MostrarBancos(); 
      }
      else{       
            control.Sql=String.format("insert into docentes values(null,'%s','%s','%s','%s')", 
    ...
    public void ModificarBanco(){
     if(txNombres.getText().length()>0 || txDni.getText().length()>0){
      ...
      control.Sql="select * from docentes where dni='"+txDni.getText()+
      "' and nombres='"+txNombres.getText()+"' and iddocentes<>'"+idBnc+"'";        
      if(!control.Verificarconsulta(control.Sql)){
      String semes=control.DevolverRegistroDto("select idsemestre from semestre where descrisemestre='"+cbSemestre.getSelectedItem().toString()+"';",1);
      String tari=control.DevolverRegistroDto("SELECT * FROM tarifas where categor=substring('"+cbTarifa.getSelectedItem().toString()+"', 1,1) and estadotar=0 and idtipocomensal=2;",1);
       control.Sql=String.format("update docentes set dni='%s',nombres='%s', genero='%s', foto='%s' where iddocentes='%s';",
       ...
       control.Sql=String.format("update consumidores set idtarifas='%s', estado='%s' where idcomensales='%s' and idsemestre='%s' and idtipocomensal='%s' ;",
                              tari,cbEstado.getSelectedIndex(),idBnc,semes,"2");
       control.EditarRegistro(control.Sql);
     ...
     if(control.fila>=0){
      idBnc=tBancos.getValueAt(control.fila,0).toString();   
       if(JOptionPane.showConfirmDialog(rootPane,"Seguro deseas eliminar","Confirmar",0)==0){
        control.Sql=String.format("delete from consumidores where idcomensales='%s' and idtipocomensal='2'", idBnc);
        control.EliminarRegistro(control.Sql);
        control.Sql=String.format("delete from docentes where iddocentes='%s'", idBnc);
        control.EliminarRegistro(control.Sql);
        Cancelar();MostrarBancos();    
       }
     }     
    ...
    public void VerBanco(){
     ...
     if(control.fila>=0){
      ...
      txFoto.setText(control.DevolverRegistroDto("select foto from docentes where iddocentes='"+idBnc+"';", 1));
...
//      cbFacultad.setSelectedItem(tBancos.getValueAt(control.fila,5).toString());
    ...
    public void BuscarBanco(){
        control.Sql="select " +
                " iddocentes,dni, nombres, if(genero='0','Masculino','Femenino'),  if(c.estado=0,'Activo','InActivo'),  monto,  categor, descrisemestre,descritipcom " +
                " from docentes d,consumidores c, tipocomensal tc, tarifas ta, semestre se " +
                " where d.iddocentes=c.idcomensales and c.idtipocomensal=tc.idtipocomensal and " +
                " c.idtarifas=ta.idtarifas and c.idsemestre=se.idsemestre " +
                " and tc.idtipocomensal=2 and se.estadosem='0' and (dni like '%"+txBuscar.getText()+"%' or nombres like '%"+txBuscar.getText()+"%');";
    ...
    public Registro_Docentes() {
     initComponents();setTitle("registro de Docentes");
     ...
     tBancos.getColumnModel().getColumn(1).setPreferredWidth(60);
     ...
     control.LlenarCombo(cbTarifa,"SELECT concat(categor,' - ',monto) FROM tarifas where estadotar='0' and idtipocomensal='2';",1);
    }
    /*************************Métodos************************/
    ...
    private void initComponents() {
        ...
        getContentPane().add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 60, 370, 150));
        ...
        jPanel4.add(cbEstado, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 110, 230, -1));

        jPanel4.add(cbFacultad, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 20, 230, -1));
        ...
        jPanel4.add(cbSemestre, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 50, 230, -1));
        ...
        jPanel4.add(cbTarifa, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 80, 230, -1));
        ...
        getContentPane().add(jPanel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(570, 60, 380, 150));
        ...
        getContentPane().add(lbFoto, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 70, 150, 140));
 ...
 if(evt.getClickCount()==2){
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        if(txDni.getText().trim().length()>0){
            ...
            JOptionPane.showMessageDialog(null, "Ingrese Datos Completos del Docente");
        }
            ...
            try {
                ...
                for (int sheetNo = 0; sheetNo < archivoExcel.getNumberOfSheets(); sheetNo++) {
                    ...
                    for (int fila = 1; fila < numFilas; fila++) {
                        ...
                        int vercantidad=Integer.parseInt(control.DevolverRegistroDto("select count(iddocentes),iddocentes from docentes where dni='"+dni+"';", 1));
                        System.out.println(nombres+"--"+dni+"--"+gen);
                        System.out.println(String.format("insert into docentes values(null,'%s','%s','%s','sinImagen');",
                                    nombres,dni,gen));
                        if(vercantidad==0){
                            control.Sql=String.format("insert into docentes values(null,'%s','%s','%s','sinImagen');",
                                    nombres,dni,gen);
                            int idpers=control.executeAndGetId(control.Sql);
                            System.out.println(control.Sql);
                            control.Sql=String.format("insert into consumidores values(null,'%s','%s','0','%s','2');", 
                                    tari,semes,""+idpers) ;
                            ...
                            System.out.println("SELECT count(*) from consumidores where idcomensales='"+idpers+"' and idtipocomensal='2' and idsemestre='"+semes+"' and estado='0';");
                            int vercom=Integer.parseInt(control.DevolverRegistroDto("SELECT count(*) from consumidores where idcomensales='"+idpers+"' and idtipocomensal='2' and idsemestre='"+semes+"' and estado='0';", 1));
                            
                            
                            if(vercom==0){
                                control.Sql=String.format("insert into consumidores values(null,'%s','%s','0','%s','2');",tari,semes,""+idpers) ;
                                control.CrearRegistro(control.Sql );
                            }
